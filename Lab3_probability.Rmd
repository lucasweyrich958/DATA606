...
**A streak of 1 means that Kobe scored a basket, followed by a miss. 0 streak length means an immediate miss.**
...
**Firstly, something seemed off with the bar plot code above, so I replaced it with base R historgram to look at the distribution. The distribution shows that for the most part, Kobe had shooting streaks between 0 and 1, but there were more than 5 streaks with 2 or 3 hits, each. The highest streak is 4.**
...
```{r}
coin_tosses = sample(coin_outcomes, size = 100, replace = TRUE, 
                          prob = c(0.2, 0.8))
table(coin_tosses)
```
**Using table() we can look at the count of each outcome, heads and tails. We can see that tails came up 78 times and heads 22 times.**
...
```{r}
sim_basket = sample(shot_outcomes, size = 133, replace = T, prob = c(.45, .55))
table(sim_basket)
```
...
The typical streak length is similar to Kobe, which makes sense, being between 0 and 1. However, there are plenty of shooting streaks that are 2, 3 or even 4 hits long. 
...
It is likely going to be quite simialr to the simulation ran above, because the probability for a hit or miss leads that distribution. So it is likely that these streaks appear at different positions throughout the trials, but that they will appear quite similar. They won't be exactly the same if we assign the simulation to a new variable. But the same variable would show the same result due to the seed I set above in the code. 
...
Plotting both histograms again
```{r}
hist(kobe_streak)
hist(sim_streak)
```

The distributions look very similar, which points to the conclusion that the Kobe does not appear to have had hot hands. The idea of a hot hand in basketball is in general not proven. But because the distribution of the simulated histogram shows an independent one, we have more evidence to show this quantitatively.
* * *
