...
samp1 %>%
  ...
  mutate(s = n /sum(n))
```
It seems that the proportion of the sample overweigths a bit more for the answer that scientists benefit them, versus don't. The population shows an 80/20 proportion while the sample shows a 92/8 proportion
...
**Insert your answer here**
I would not expect the samples to be the exact same. The two samples will still be similar, such that the answer "Benefits" will still be higher than "Doesn't Benefit", but it will not be the exact same, unless they picked the same seed as that assures the same sample to be drawn.
...
samp2 %>%
  ...
  mutate(p_hat = n /sum(n))
```
This sample looks certainly a bit different to the first sample drawn, but there is still somewhat an 80/20 proportion. If we took samples with n = 100 or 1000, we would find that  we get more and more to the population distribution, which is 80/20. The larger the sample the more accurate it becomes, but there are often constraints on that (access to people in social sciences, for example)
...
**Insert your answer here**
If by elements it is meant how many samples, there are 15000. If by elements it is meant the range of proportion, it ranges from about 0.02 to over 0.4. This means that some samples had an almost 50/50 distribution, while some samples had an 99/1 distribution. This shows how important sample size is, and also to utilize proper sample techniques. Looking at the midpoint, it appears to be right at 0.2, which is the population proportion, confirming the central limit theorem. 
```{r}
ggplot(data = sample_props50, aes(x = p_hat)) +
  geom_histogram(binwidth = 0.02) +
  geom_vline(xintercept = median(sample_props50$p_hat), color = "red", linewidth = 0.5) +
  labs(
    x = "p_hat (Doesn't benefit)",
    title = "Sampling distribution of p_hat",
    subtitle = "Sample size = 50, Number of samples = 15000"
...
sample_props_small
```
There are 24 observations and each represents the proportion of a sampling of 10 observations from the global monitor dataset. 
...
**Insert your answer here**
Each observation in the sample distribution represents one sample drawn, of the respective size specified, from the population. It shows that when using only 10, the distribution is quite scattered and can include proportion from 0.1 to 0.4, but the higher the sample size, the closer the full sample distribution gets to the population proportion of 0.2. This is because of the central limit theorem where the larger the sample the closer it gets to the population mean, which in this case corresponds to the proportion. The values do not really change when increasing the number of simulations, at least in terms of whether the distributions stays around the population proportion. In fact, increasing the number of simulations with a low sample size can actually increase the chance that some proportions are completely off, such as 50/50.
...
```
Looking at a sample of n = 15, the proportion of respondends who think scientists benefit them is 0.733, which is lower than the population of 0.8.
...
Based on the sampling distribution of size 15 with 2000 samples, it seems to be between 0.75 and 0.85, so there is some range to it but it would allow to take a solid guess. It is within the actual proportion of 0.8, but not necessarily fully accurate.
...
ggplot(data = sample_props150, aes(x = p_hat)) +
  geom_histogram(binwidth = 0.02)
```
When looking at a sample size of 150, compared to 15, firstly, the proportions seems much more centered around the population proportion of 0.8. The minimum here is about 0.65 and the maximum is about 0.93, whereas with a sample size of 15 the minimum was 0.3, so much more left skewed and not fully bell-shaped. With the larger sample size we get closer to that normal distribution centered around the true population proportion.
...
A sampling distribution with a tight spread is better because we can more certain that our random sample drawn from that is closer to the true value compared to a wide spread.
* * *
