...
```
The proportion of the sample drawn above is 0.617.

1.  Would you expect another student's sample proportion to be identical to yours? 
    Would you expect it to be similar? Why or why not?

**Insert your answer here**
Unless we set the same seed, we unlikely will have the exact same proportion. It is possible that we will have a similar distribution, however, even that is not super certain because we're drawing a sample of 60 of a population of 100000.
...
**Insert your answer here**
95% confidence means that if we were to repeatedly sample, 95% of our samples will include the true population mean. 
...
samp %>%
  ...
  get_ci(level = 0.95)
```
Looking at the output here again, we can see that the CI of the bootstrapping does contain the population proportion. Its lower limit is 50% and upper limit is 73%. 
...
**Insert your answer here**
I would expect 95% of the CIs to capture the population mean, as that is what we specified by selecting the 95% CI as upper and lower limits. If we were using 90% CI, for example, we'd be more strict of capturing the population mean, but also accept that we won't capture it as often.
...
**Insert your answer here**
```{r, out.width = '100%'}
knitr::include_graphics("/Users/Lucas/Desktop/tv.png")
```
```{r}
3/50
```
I am not quite sure what is meant by include my own plot. I am not sure how to plot what's shown in the app without also recreating the sampling distribution (I thought that's why we had the app)? Alternatively, I included a screenshot of the plot. 
What can be seen here is that there are 3 of the 50 CIs do not include the population proportion, which is 6%, so it is a bit higher than the 95% CI, but it is very close. It is expected to vary a bit.
* * *
...
**Insert your answer here**
If we would pick a CI of 90%, this would be a narrower confidence interval because we would only be 90% confident to include the population mean. This means it is more stringent.
...
samp %>%
  ...
  get_ci(level = 0.90)
```
Going off of the answer above, using a CI of 90% will be more stringent, giving a narrower range. In this case, the lower and upper bound is 0.52 and 0.72, which is narrower than the 95% interval. This still includes the population proportion in this case.
...
**Insert your answer here**
```{r, out.width = '100%'}
knitr::include_graphics("/Users/Lucas/Desktop/tc2.png")
```
```{r}
5/50
```
Again, included a screenshot of the app's plot. The proportion here shows 5 of the CIs to not include the population proportion. This is exactly in line with the 90% CI, and because we have a narrower range here compared to the 95% CI, it is unsurprising that there are more CIs excluded here.
...
**Insert your answer here**
Using a CI of 99%, the widths will be wider than with the previous CIs. This means that we'll include the population proportion more often, but the range will also be wider. 
```{r}
samp %>%
  specify(response = climate_change_affects, success = "Yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  ...
  get_ci(level = 0.99)
```
As can be seen, now the range is quite large, over 50% in fact. This increases our ability to include the population proportion, but of course also increases the risk for error. Using the app, it actually shows all of the CIs to include the population proportion.
...
**Insert your answer here**
Using the 99% CI, and using 400 as sample size instead of 60 actually leads to one CI not capturing the population proportion, but using a smaller sample size of 10 still includes all CIs. 
...
**Insert your answer here**
No, the number of bootstraps should not affect the width of the CI because it should not impact the standard error a lot. Even though bootstrapping resamples, it still uses the same data.
