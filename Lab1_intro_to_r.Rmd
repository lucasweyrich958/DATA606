...
```
  ...
  It appears that there was a more consistent growth trend after the year 1660, approximately. Prior to that, it seemed that there was a dip in batisms for girls, which may be attributable to a certain religious or political event, like a war or a recession. But generally speaking, after 1660, a strong upward trend is observable. We could assess the growth rate per year, for example, to get a better idea as to how exactly this growth looks like.
...
```
  ...
  The line graph shows the ratio of boys over time, which has a really tight scale on the y-axis, which leads to a messy looking line graphs. It appears that the ration of boys being baptized ranges between 0.50 and 0.54, but varying quite a bit between each year. An average across the years would probably show 0.52 because it keeps hovering around that number. A clear patern can't be recognized, though.
...
Using the head() and glimpse() command, it appears that the data frame shows the birth counts of boys and girls in the mid and late 20th century, going into the 21st. It is structured the same way, columns for year, boys and girls.
...
Using summary(), which is the same as the summarize code shown above, shows several summary metrics, such as mean, min and max. The minimum for boy and girls is 1211684 and 1148715, respectively, which is much higher of counts than the previous dataset.
...
Arbuthnot's observation hold strong throughout 1940 to the early 2000s, albeit a declining proportion that seems to be trending toward a equal 50/50 split between boys and girls.
...
The year with the most total births in the US was, with 4,268,326 births, 1961.
