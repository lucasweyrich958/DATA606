...
This dataset contains many variables, 123 to be exact, most of which are numerical. The ISO_code is also a part of the dataset, which is nice because this would allow for map-visualizations. 
...
To visualize this relationship, a scatterplot would be most appropriate. Given the scatterplot, it appears as if the relationship is not fully linear, but rather squared. Therefore, it would be reasonable to approach it in both ways--linear and squared. This is because it seems like countries with a very low PF score also have a very PF expression score, and the same accounts for the upper extremes, but the middle is a bit more mixed. However, a linear prediction would do likely just as good, since the curvature is not that strong.
...
As discussed already a bit above, it seems that the values in the middle for each variables are more spread. Such that it is a bit more bloated up in the middle, whereas both extremes are more centered. Additionally, the relationship clearly appears to be a positive: when PF score increases, PF expression also rises. This direction is also very strong.
...
The lowest I have gotten was a SS of 1041.57. This is similar to some people I pinged on Slack.
...
```{r}
lm_exp_hf = lm(hf_score ~ pf_expression_control, data = hfi)
summary(lm_exp_hf)
```
...
The linear model of PF expression control prediciting human freedom score is positive, since the estimate is positive. Additionally, for every unit PF expression control rises, the human freedom score rises by 0.35, a somewhat strong positive relationship. Additionally, the R2 being 0.58 means that 58% of the variance is explained, which is also quite a good score.
...
Looking only at the regression line, someone would predict the PF score to be approximately 8. This is quite in the middle of the actual values, which means that it is neither an over- nor underestimate. However, the residuals appear to be still a bit high since the data is spread out.
...
It appears that there are more residuals for lower fitted values than greater ones. Additionally, it appears that there are more residuals below 0 than above, which could mean that the model overestimated the values more often than underestimated them with the best-fit line.
...
***The ggplot histogram did not seem to work, so I used base-R histogram to assess the relationship
...
Yes. The requirement for nearly normal residuals appears to be met, since the histogram shows a Gaussian distribution and the Q-Q plot shows a somewhat straight line, except for a few extremes, which is forgivable.
...
Yes. The importance is that the points are equally spread out across the whole graph, and that there is no noticable gap or direction. Therefore, the variability condition is met also.
...
```{r}
ggplot(data = hfi, aes(x = pf_ss_disappearances, y = ef_legal_integrity)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE)
lm_legal_disapp = lm(ef_legal_integrity ~ pf_ss_disappearances, data = hfi)
summary(lm_legal_disapp)
```

I chose the variables for disappearances and legal integrity, because I figured there would be a negative relationship, such that lower integrity would lead to greater disappearences, but there seems to be a positive relationship. However, the scatterplot shows that the relationship is certainly not linear.
...
The R2 for for legal integrity and disappearances is only 0.09, which means barely 10% of the variance is explaine, while the R2 for expression control and personal freedom was much higher with 0.58.
...
It has to be the one I used in the example above, shown again here, because how come there are more disappearances with higher legal integrity? Perhaps the way integrity is measured is by what the government wants, and not by how fair it is, but I would have imagined this relationship to be negative.
...
```
```{r}
hist(lm_legal_disapp$residuals)
ggplot(data = lm_legal_disapp, aes(sample = .resid)) +
  stat_qq()
ggplot(data = lm_legal_disapp, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals")
...
However, given the diagnostic plots, it appears that this model is not really anyways, because the Q-Q plot does not show a straight line, and is instead looking like stairs. Additionally, the Fitted vs. Residuals plot does not show scatter across the graph, but instead shows a direction toward the right. This means that the residuals are not equally distributed, which makes the model validity questionable. 
* * *
