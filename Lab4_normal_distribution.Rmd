...
Both distributions seem to be more right-skewed (e.g., with a long right tail). Therefore, they would not be considered normally distributed. It alsos seems like that the McDonald's dataset is more right-skewed than the Dairy Queen set.
...
This plot convinces me a bit more that the Dairy Queen dataset least, it quite normally distributed, although it still seems to be a bit right-skewed.
...
While many of the simulated plots also seem to diverge from the straight line, especially towards the higher values, it still seems as though the real data is diverging a bit more than the simulated data. But generally I think it is safe to say that the Dairy Queen data is normally distributed.
...
Looking at the seven simulated QQ plots in comparison to the McDonald's QQ plot, it seems that here the data is more skewed than the Dairy Queen distribution. This conforms with the notion that was seen in the histogram already.
...
(1) The FDA recommends that people limit their sodium intake to 2300mg/day. Therefore, it would be interesting to know what the probability is that one single menu item from Arby's will be greater than half (1150mg) of that maximally recommended intake. To get there, we'll calculate the mean and sd for Arby's sodium content and simulate the normal distribution, then we'll calculate the actual probability. 

```{r}
arbys = fastfood %>%
  filter(restaurant == 'Arbys')
a_mean = mean(arbys$sodium)
a_sd = sd(arbys$sodium)

1 - pnorm(q = 1150, mean = a_mean, sd = a_sd)
arbys %>% 
  filter(sodium > 1150) %>%
...
(2) Another interesting question would be to see how many of Subway's menu items fall above 500 calories. That is a lot for one single item, but Subway has the general conception of being one of the healthier fastfood restaurants (at least in Germany), so seeing what is the probability of that can point to whether that conception actually holds true. See below.
...
1 - pnorm(q = 500, mean = s_mean, sd = s_sd)
subway %>% 
  filter(calories > 500) %>%
  summarise(percent = n() / nrow(subway))
hist(subway$calories)
```

It seems like that the probability of Arby's sodium content being higher than 1150mg (half the maximum daily recommended amount) is closer in agreement with the simulated normal distribution's probability than the probability that a Subway item is greater than 500 calories with its normal distribution's probability. For my own sake I plotted for both a histogram and it shows that the Arby's distribution at least more dense, while the Subway distribution is a bit more spread out, possibly explaining the that discrepancy.
...
Looking at it only visually, it appears that Burger King's sodium content is most closely normally dsitributed. Because the histogram shows a peak (or two peaks) somewhat in the middle, and the QQ-plot also shows the least amount of extreme divergence, compared to the one from Chick Fil-A, for example. 
...
It is possible that some of the items have extreme high sodium content, like fries do, and the rest seems to be a bit more reasonable. This causes some outliers that can lead to a stepwise pattern.
...
ggplot(sonic, aes(sample = total_carb)) +
  geom_line(stat = "qq")
ggplot(fastfood, aes(x = total_carb)) +
  geom_histogram()

```
The total carbohydrates for Sonic appear to be somewhat normally distributed, albeit being a bit right-skewed. This means that it has a bit of a right tail, primarily due to one or two items that appear to be above 150 carbs.
* * *
